...
{
    if(printing) printf("we start the insert exec attempt.\n");
    ...
    counter = 0;
    if(printing) printf("t#:%d i#:%d\n", thrdNumVal, itrNumVal);

    // if list is empty
    if(list->next == NULL)
    {
        if(printing) printf("list->next NULL, INSERT of empty list\n");
    ...
    if(printing) printf("list not empty, ascending insert attempt\n");
    ...
    if(printing) printf("we found insert spot\n");
    ...
    if(curr == NULL)
    {
        ...
        element->next = NULL;
        if(printing) printf("list INSERTED at end\n");
    }
    else
    {
        previous->next = element;
...
int SortedList_length(SortedList_t *list)
{
    ...
    if(printing) printf("final len:%d\n", len);
    return len;
}