...
void* listThreads(void *input)
{
    ...
    if(len != itrNum)
    {
        fprintf(stderr, "List improperly created.\n");
        exit(2);
    }
...
int main(int argc, char* argv[])
{
    ...
    {
        ret = pthread_create(&threads[i], NULL, &listThreads, (void *) elements + itrNum * i);
        if(ret)
        {
